...
ggplot(
  ...
  theme_minimal() + 
  labs(
    title = "Histograma de la superficie de la casa",
...
ggplot(
  ...
  theme_minimal() + 
  labs(
    title = "Superficie de la casa frente al precio de venta",
    caption = "MSMK: taller de machine learning"
  ) 
...
?model.matrix
train <- model.matrix(~.-Id, data = train)
...
id_train <- sample(1:nrow(train), size = 0.7*nrow(train))
...
# el precio de venta
reg_lineal1 <- lm(SalePrice~GrLivArea, data = as.data.frame(train))
...
test_error$predict_reglineal1 <- predict(reg_lineal1, newdata = as.data.frame(test))
...
modelo_error <- tibble(
  modelo = "reglineal1",
  error = error(test_error$SalePrice, test_error$predict_reglineal1)
)
...
ggplot(data = test_error,
       aes(
         ...
         y = predict_reglineal1 - SalePrice
       )
) + 
  geom_point(color = "steelblue",
             alpha = 0.4) + 
...
reg_lineal2 <- lm(SalePrice~.-1, data = as.data.frame(train))
...
reglineal_glmnet <- glmnet(x = train[, 1:261], y = train[, "SalePrice"])

test_error$predict_reglineal_glmnet <- predict(reglineal_glmnet, newx = test[, 1:261], s = 0.01)

modelo_error <- bind_rows(modelo_error,
                          list(modelo = "reglineal_glmnet", 
                               error = error(test_error$SalePrice, test_error$predict_reglineal_glmnet)
                               )
                          )
...
gradient_boost <- xgboost(data = train[, 1:261], 
                          label = train[,"SalePrice"],
                          nrounds = 100
                          )

test_error$gradient_boosting <- predict(gradient_boost, test[,1:261])



modelo_error <- bind_rows(modelo_error,
                          list(modelo = "gradient_boosting", 
                               error = error(test_error$SalePrice, test_error$gradient_boosting)
                               )
                          )
...
reglineal_glmnet_log <- glmnet(x = train[, 1:261], y = log(train[, "SalePrice"]))

test_error$predict_reglineal_glmnet_log <- exp(predict(reglineal_glmnet_log, newx = test[, 1:261], s = 0.01))
...
gradient_boost_log <- xgboost(data = train[, 1:261], 
